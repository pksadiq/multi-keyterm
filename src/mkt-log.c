  ...
  if (use_color)
    {
      switch ((int)log_level)        /* Same colors as used in GLib */
        ...
        case MKT_LOG_LEVEL_TRACE:  return "   \033[1;36mTRACE\033[0m";
  ...
  else
    {
      switch ((int)log_level)
        ...
        case MKT_LOG_LEVEL_TRACE:  return "   TRACE";
        default:                   return " UNKNOWN";
        }
    }
}
...
mkt_log_write (GLogLevelFlags   log_level,
               const char      *log_domain,
...
static GLogWriterOutput
mkt_log_handler (GLogLevelFlags   log_level,
                 const GLogField *fields,
                 gsize            n_fields,
                 gpointer         user_data)
{
  ...
  if (any_domain)
    return mkt_log_write (log_level, log_domain, log_message,
  ...
  if (g_once_init_enter (&initialized))
    {
      ...
      g_log_set_writer_func (mkt_log_handler, NULL, NULL);
      g_once_init_leave (&initialized, 1);
      atexit (mkt_log_finalize);
    }
}

void
mkt_log_increase_verbosity (void)
{
  verbosity++;
}

int
mkt_log_get_verbosity (void)
{
  return verbosity;
}
